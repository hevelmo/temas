$(function() {
	...
	$("#colorpicker").spectrum({
		...
		chooseText: "select",
		move: function(color) {
			updateBg3Color(color);
		},
		change: function(color) {
			updateBg3Color(color);
		}
	});
	function updateBg3Color(color) {
		$($("#emailframe").contents().find('.bg3_color')).css("background-color", color.toHexString());
	}
	...
	$("#colorpicker5").spectrum({
		...
		chooseText: "select",
		move: function(color) {
			updateBg4color(color);
		},
		change: function(color) {
			updateBg4color(color);
		}
	});
	function updateBg4color(color) {
		$($("#emailframe").contents().find('.bg4_color')).css("background-color", color.toHexString());
	}
	...
	$("#colorpicker7").spectrum({
		...
		chooseText: "select",
		move: function(color) {
			updateBg5color(color);
		},
		change: function(color) {
			updateBg5color(color);
		}
	});
	function updateBg5color(color) {
		$($("#emailframe").contents().find('.bg5_color')).css("background-color", color.toHexString());
	}
	...
	$(".save-template").on("click",function(e){
		...
		var htmlcontent = $("#emailframe").contents().find("#promail").html();
		
		var htmlcontent2 = $(htmlcontent).filter(".action-btn").remove();
		
		$(".clone").html(htmlcontent);
		$(".clone").find(".action-btn").remove();
		
		$(".clone li").children().unwrap();

		$('#download-form').find('.html-code').val($(".clone").html()).end().submit();
